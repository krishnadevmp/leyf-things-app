...
export const GoalList = () => {
  ...
  const handleFilterChange = (event: SelectChangeEvent<GoalFilters['status']>) => {
    setFilters({ status: event.target.value as GoalFilters['status'] });
  };

  const handleSortChange = (_: MouseEvent<HTMLElement>, value: GoalFilters['sortBy'] | null) => {
    if (value) {
      setFilters({
        sortBy: value,
          ...
          <CardContent>
            <Stack spacing={2}>
              ...
              <Typography variant="body2" color="text.secondary">
                {completedCount} of {goals.length} goals completed ({Math.round(progress)}%)
              </Typography>
            </Stack>
          </CardContent>
        </Card>
            ...
            <Grid item xs={12} sm={6} md={8}>
              ...
              >
                <ToggleButton value="targetDate">
                  <SortIcon />
                  Target Date {filters.sortBy === 'targetDate' && (filters.sortOrder === 'asc' ? '↑' : '↓')}
                </ToggleButton>
                <ToggleButton value="createdAt">
                  <SortIcon />
                  Created Date {filters.sortBy === 'createdAt' && (filters.sortOrder === 'asc' ? '↑' : '↓')}
                </ToggleButton>
              </ToggleButtonGroup>
            </Grid>
          </Grid>
                ...
                <Stack spacing={2}>
                  <Box display="flex" justifyContent="space-between" alignItems="center">
                    ...
                    <Box>
                      ...
                      <IconButton onClick={() => setEditingGoal(goal)} color="primary">
                        <EditIcon />
                      </IconButton>
